...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            other = place.ant
            if other.is_container and other.can_contain(self):
                other.store_ant(self)
            elif not other.is_container and self.is_container and self.can_contain(other) :
                place.ant = self
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        idx = self.place
        cnt = 0
        while idx.entrance != None and cnt <= self.upper_bound:
            if idx.bees != [] and not idx.is_hive and cnt >= self.lower_bound:
                return random_bee(idx.bees)
            idx = idx.entrance
            cnt += 1
        if idx.bees != [] and not idx.is_hive and cnt >= self.lower_bound and cnt <= self.upper_bound:
            return random_bee(idx.bees)
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown > 0:
            ...
            if self.place.bees != [] :
                randbee = random.choice(self.place.bees)
...
def slow_action(bee, gamestate):
    if bee.slowlength > 0 :
        ...
        if gamestate.time % 2 ==0 :
            bee.origin_action(gamestate)
    else :
        bee.origin_action(gamestate)


def change_into_slow(bee,slowlength) :
    if not hasattr(bee, 'slowlength') :
        bee.origin_action = bee.action
    bee.slowlength = slowlength
    def slow_status(gamestate) :
        nonlocal bee
        slow_action(bee,gamestate)
    bee.action = slow_status
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 1
        if target :
            change_into_slow(target, self.SlowTime)
        # END Problem EC 1
...
class LaserAnt(ThrowerAnt):
    ...
    food_cost = 10
    base_damage = 2
    damage_weakened = 0.25
    damage_decrease = 0.0625
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem EC 4
    implemented = True   # Change to True to view in the GUI
    # END Problem EC 4
    ...
    def insects_in_front(self):
        ...
        dict = {}
        cnt = 0
        while not pre.is_hive :
            for bee in pre.bees :
                dict[bee] = cnt
            if pre.ant and (pre.ant is not self):
                dict[pre.ant] = cnt
            pre = pre.entrance
            cnt += 1
        return dict
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        damage = self.base_damage- self.damage_decrease * self.insects_shot - self.damage_weakened * distance
        damage = max(0,damage)
        return damage
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scared_length > 0 :
            ...
            if destination.is_hive or (hasattr(self,'slowlength') and self.slowlength > 0 and gamestate.time % 2 ==1) :

...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.define(first,scheme_eval(rest.first,env))
        return first
        # END PROBLEM 4
        ...
        #print(exp)
        procedure = do_lambda_form(exp, env)
        env.define(procedure_name,procedure)
...
def do_and_form(expressions, env):
    ...
    while exp != nil:
        if exp.rest :
            tmp = scheme_eval(exp.first, env,False)
            if tmp is False: return False
            exp = exp.rest
        else :
            tmp = scheme_eval(exp.first, env,True)
            if tmp is False: return False
            exp = exp.rest
    return tmp
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    while exp != nil :
        if exp.rest :
            tmp = scheme_eval(exp.first, env, False)
            if tmp is not False: return tmp
            exp = exp.rest
        else :
            tmp = scheme_eval(exp.first, env, True)
            if tmp is not False: return tmp
...
def make_let_frame(bindings, env):
    ...
    while binding is not nil :
        if (len(binding.first) != 2):
            raise SchemeError('bad bindings list in let form')
        validate_form(binding.first,0)
        name , val = binding.first.first , scheme_eval(binding.first.rest.first,env)
        if not scheme_symbolp(name):
            raise SchemeError('bad scheme name in let form')
        if name in dicti:
            raise SchemeError('{0} has already bound'.format(name))
        names.rest = Pair(names.first,names.rest)
        names.first = name
        vals.rest = Pair(vals.first,vals.rest)
        vals.first = val
        dicti[name] = val

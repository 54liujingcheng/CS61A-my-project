...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def about_word(element):
        s = split(remove_punctuation(lower(element)))
        #print (s)
        #print(subject)
        for word in subject:
           # print(word)
...
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words), len(source_words))):
        if typed_words[i] == source_words[i]:
            cnt += 1
    return float(cnt)/len(typed_words)*100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    return min(word_list,key=lambda w: diff_function(w, typed_word, limit))
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed == source : # Feel free to remove or add additional cases
        ...
        for i in range(min(len(typed), len(source))):
            if typed[i] != source[i]:
                break
        if i == len(typed)-1 or i == len(source)-1:
            return min(limit+1,abs(len(source)-len(typed)))
        add = typed[0:i] + source[i] + typed[i:]
        remove = typed[0:i] + typed[i+1:]
        substitute = typed[0:i] + source[i] + typed[i+1:]
        return min(limit+1,minimum_mewtations(add, source, limit)+1,minimum_mewtations(remove, source, limit)+1,minimum_mewtations(substitute, source, limit)+1)
        # BEGIN
        "*** YOUR CODE HERE ***"
        # END
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] != source[i]:
            break
    d={'id' : user_id, 'progress' : (i)/len(source) if typed[i] != source[i] else (i+1)/len(source)}
    upload(d)
    return d['progress']
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[tpp[j][i+1]-tpp[j][i] for i in range(len(tpp[j])-1)] for j in range (len(tpp))]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    fuck =[[] for i in player_indices]
    for i in word_indices:
        p = min(times[x][i] for x in player_indices)
        for j in player_indices:
            if p in times[j]:
                fuck[j].append(words[i])
                break
    return fuck
    # END PROBLEM 10
